...
/* WEB SOCKET */
	...
	var baseURL = document.URL.split('/addlocations')[0].split('http://')[1];
...
/* INITIATE MAP OBJECT */
	...
	// LAUNCH MAP OBJ
		map.launch = function() {
			try {
			// CREATE GLOBAL OBJS
				window.sendLoc = function(n) {
					var apiRoute = 'api/v1/submit_location';
					var sendData = {
						"web_token": map.config.web_token,
						"base_layer": map.active,
						"gps_lat": map.markers[n].getLatLng().lat,
						"gps_long": map.markers[n].getLatLng().lng,
						"description": $('.description').val()
					};
					var result = ipsAPI(apiRoute,sendData);
					if (result.success) {
						// CLEAR AND REPLACE FEATURES
			...
			// CREATING MARKERS
				this.on('click', function(e) {
					...
					if (this.markers.length < 1) {
						...
						var form = '<b> Description: <br> <input type="text" name="descripton" value="" id="description" class="description"> </b> <br>';
						form += '<b> <button type="button" onclick="sendLoc(' + marker.id + ');">Submit</button> </b>';
